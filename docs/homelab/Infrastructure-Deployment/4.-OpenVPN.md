...
*📅 Published: 2026-04-16 01:56 | 🔄 Last Updated: 2026-05-30 11:51*
<!-- notion-metadata-end -->
---
...
5. Server Certificate:
	- Descriptive Name: `SOC_VPN_Cert` -> _Add new Certificate_.
...
Traffic from clients to server: By default, pfSense blocks inbound connections from the Internet to the internal network. Checking this option allows pfSense to permit inbound UDP traffic on the previously configured port 1194.


Traffic from clients through VPN: Once the laptop successfully connects, it is assigned a virtual IP address within the OpenVPN interface. Without this rule, the traffic would be stuck there and unable to route anywhere else.
...
- `<ca>` **(CA Certificate - CA’s public key):** Contains the Public Key of the Certificate Authority. The laptop uses this to verify the digital signature on the Server's Certificate, proving the server is authentic and trusted.
- `<cert>` **(Client Certificate - SOC Analyst public key):** Contains the SOC Analyst's Public Key, coupled with the CA's digital signature verifying your identity. The client presents this to the Server to prove who they are.
- `<key>` **(SOC Analyst Private Key):** Contains the SOC Analyst's highly sensitive Private Key. This is used for cryptographic proof of identity and securely exchanging the session keys.
